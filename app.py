...
# ==========================================
    ...
    # Дефолты для cases (кейсы/отгрузки)
    if mode == 'cases' and 'CASE_MAIN_TITLE' not in _d:
        _d['CASE_MAIN_TITLE'] = 'Металл с гарантией: проверка по ГОСТ и полный пакет документов при отгрузке'
    if mode == 'cases' and 'CASE_TASK' not in _d:
        _d['CASE_TASK'] = 'Недостаточная толщина стенки, отсутствие сертификатов или дефекты поверхности могут остановить стройку на недели. В «Стальметурал» мы внедрили трехступенчатый контроль отгрузки, чтобы вы спали спокойно.'
    if mode == 'cases' and 'CASE_STEPS' not in _d:
        _d['CASE_STEPS'] = '<p>— <strong>Замеры перед погрузкой:</strong> Проверяем толщину стенки и параметры металла на соответствие ГОСТу.</p><p>— <strong>Полная документация:</strong> Пакет оригинальных сертификатов передается водителю сразу.</p><p>— <strong>Точность до миллиметра:</strong> Собственный цех резки на ЧПУ гарантирует, что детали встанут как влитые.</p>'
    if mode == 'cases' and 'CASE_RESULT' not in _d:
        _d['CASE_RESULT'] = 'Ваш объект не будет простаивать из-за брака.'
    # ─────────────────────────────────────────────────────────────────────────
